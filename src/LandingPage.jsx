...
const CHANCES_PER_DAY = 5; // ← change this to set attempts allowed per device, per day
...
function TenSecondChallenge({ debug = false }) {
  ...
  function saveTries(next) {
    setTriesCount(next);
    try {
      localStorage.setItem(triesKey, String(next));
    } catch {}
  }

  function handleStartStop() {
    // block new starts when out of attempts
    if (!running && triesCount >= CHANCES_PER_DAY) return;
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4" data-testid="ten-sec-root">
      ...
      {/* Winner Booking Modal — mobile-friendly wrapper */}
      {showWinModal && (
        <div
...
export default function NiBinGuyLandingPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      {/* 10-Second Challenge Modal — mobile-friendly wrapper */}
      {showChallenge && (
        <div
      ...
      {/* (other sections below as you like) */}
    </div>
  );
}
