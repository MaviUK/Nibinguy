...
/* ───────── TenSecondChallenge (no test mode) ───────── */
...
function TenSecondChallengeV2({ debug = false }) {
  const [elapsedCs, setElapsedCs] = React.useState(0);
  const [running, setRunning] = React.useState(false);
  const [hasTriedToday, setHasTriedToday] = React.useState(false);
  const [alreadyClaimedToday, setAlreadyClaimedToday] = React.useState(false);
  ...
  const [winSubmitStatus, setWinSubmitStatus] = React.useState("idle");

  const winAddressRef = React.useRef(null);
  const winSelectedPlaceRef = React.useRef(null);
  const [winPlaceId, setWinPlaceId] = React.useState(null);
  ...
  // Spacebar should NOT trigger while typing or when the win modal is open
  React.useEffect(() => {
    const onKeyDown = (e) => {
  ...
  }, [running, hasTriedToday, alreadyClaimedToday, showWinModal]);
  ...
  function handleStartStop() {
    if (hasTriedToday || alreadyClaimedToday) return;
    ...
    if (!running) {
      ...
      if (finalCs === 1000) {
        localStorage.setItem(`tensec_winner_${todayKey}`, "1");
  ...
  // Attach Places to the winner address when modal opens (optional)
  React.useEffect(() => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4" data-testid="ten-sec-root">
      ...
      <style>{`.input { @apply w-full rounded-xl border border-neutral-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500; }`}</style>
    </div>
  );
}
